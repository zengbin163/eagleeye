...
public class DubboKeeperMonitorService implements MonitorService {
	...
	public void collect(URL statisticsURL) {
		...
		statistics.setConcurrent(
				(long) statisticsURL.getParameter(MonitorService.CONCURRENT, 1));
		if(statistics.getConcurrent()==0){
			statistics.setConcurrent(1L);
		}
        ...
        statistics.setElapsed(
				(long) (statisticsURL.getParameter(MonitorService.ELAPSED, 0) / totalCount));
        statistics.setInput(
				(long) (statisticsURL.getParameter(MonitorService.INPUT, 0) / totalCount));
        statistics.setOutput(
				(long) (statisticsURL.getParameter(MonitorService.OUTPUT, 0) / totalCount));
