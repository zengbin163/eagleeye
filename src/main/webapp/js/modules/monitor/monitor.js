...
monitor.controller("applicationTop200",function($scope,$httpWrapper,$routeParams,$breadcrumb,$menu){
    ...
    $scope.app.success={};
    $scope.showType='chart';
    var loadApplicationOverview=function(text){
        $httpWrapper.post({
            url:"monitor/"+$scope.name+"/"+$scope.dayRange+"/overview.htm",
            data:"key="+text,
            config:{ headers: { 'Content-Type': 'application/x-www-form-urlencoded'}},
            success:function(data){
                var fi = data.faultItems;
                var si = data.successItems;
                var ft = 0;
                var st = 0;
...
//                generateOptions($scope.app,data.concurrentItems,'concurrent','并发',$scope.dayRange);
                generateOptions2($scope.app,data.faultItems,'fault','失败次数',$scope.dayRange);
                generateOptions2($scope.app,data.successItems,'success','成功次数',$scope.dayRange);
            }
        })
    }
    $scope.switchView = function(type){
    ...
    $scope.searchService = function (text) {
        loadApplicationOverview(text);
    }
})
...
var generateXAxisData2=function(items){
    var xAxisData = [];
    for(var i=0;i<items.length;i++){
        var date = new Date(items[i].timestamp);
        var item = items[i].method
        xAxisData.push(item);
    }
    return xAxisData;
}
var generateOptions2=function(app,values,prop,name,dayRange){
    var options = {};
    if(!values){
        app[prop]=options;
        return;
    }
    options.title="全部数据的"+name+"TOP200";
    options.subTitle="来自应用端";
    options.seriesConfig=[{name:name,data:generateMainData(values,prop)}];
    options.xAxis=generateXAxisData2(values);
    app[prop]=options;
}
