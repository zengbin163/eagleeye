...
import com.dubboclub.dk.storage.api.model.ApplicationInfo;
import com.dubboclub.dk.storage.api.model.MethodMonitorOverview;
import com.dubboclub.dk.storage.api.model.ServiceInfo;
import com.dubboclub.dk.storage.api.model.StatisticsOverview;
...
public class MonitorController {
    ...
    public @ResponseBody Integer queryInterval(){
        return Integer.parseInt(ConfigUtils.getProperty("monitor.collect.interval", "60000"));
    }
    ...
    public @ResponseBody
    StatisticsOverview queryApplicationOverview(@PathVariable("application")String application, @PathVariable("dayRange")int dayRange, @RequestParam(value = "key", required = false) String key){
        long currentTime = System.currentTimeMillis();
        if (Objects.equals(key, "")) {
            key = null;
        }
        return statisticsStorage.queryApplicationOverview(application,dayRange==0?dayRange:currentTime-(dayRange*ONE_DAY),currentTime,key);
    }
